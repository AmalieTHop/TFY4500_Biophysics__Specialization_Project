...
class IVIMfit:
    ...
    def run_fit(self, true_params=[], verbose=1):
        ...
        if self.multithreading:
            ...
            self.ivim_metrics_all_voxels = pool.map(partial(fit_warpper, approach = self.approach), fit_func_args)
            pool.close()
            pool.join()
        else:
            # ---- on one worker ----
            ...
            self.ivim_metrics_all_voxels = list(map(partial(fit_warpper, approach = self.approach), fit_func_args))
...
def main(dwi_fname, bval_fname, mask_fname, model, ofolder, multithreading):
    ...
    # load data
    dwi = nib.load(dwi_fname).get_fdata() #
    ...
    ivim_fit = IVIMfit(bvals=bvals,
                       voxels_values=dwi[mask > 0, :], #orig: voxels_values=dwi[mask > 0, :],
    ...
    S0_map_nii = nib.Nifti1Image(S0_map.copy(), mask_nii.affine, mask_nii.header); nib.save(S0_map_nii, ofolder+"/S0_map.nii")
    D_map_nii = nib.Nifti1Image(D_map.copy(), mask_nii.affine, mask_nii.header); nib.save(D_map_nii, ofolder+"/D_map.nii")
    FivimXDstar_map_nii = nib.Nifti1Image(FivimXDstar_map.copy(), mask_nii.affine, mask_nii.header); nib.save(FivimXDstar_map_nii, ofolder+"/FivimXDstar_map.nii")
    AIC_map_nii = nib.Nifti1Image(AIC_map.copy(), mask_nii.affine, mask_nii.header); nib.save(AIC_map_nii, ofolder+"/AIC_map.nii")
    R2_map_nii = nib.Nifti1Image(R2_map.copy(), mask_nii.affine, mask_nii.header); nib.save(R2_map_nii, ofolder+"/R2_map.nii")
    exception_map_nii = nib.Nifti1Image(exception_map.copy(), mask_nii.affine, mask_nii.header); nib.save(exception_map_nii, ofolder+"/exception_map.nii")
    if model != 'FivimXDstar':
        Fivim_map_nii = nib.Nifti1Image(Fivim_map.copy(), mask_nii.affine, mask_nii.header); nib.save(Fivim_map_nii, ofolder + "/Fivim_map.nii")
        Dstar_map_nii = nib.Nifti1Image(Dstar_map.copy(), mask_nii.affine, mask_nii.header); nib.save(Dstar_map_nii, ofolder + "/Dstar_map.nii")
    if model == '1shot_initD_noise':
        ...
        noise_map_nii = nib.Nifti1Image(noise_map.copy(), mask_nii.affine, mask_nii.header); nib.save(noise_map_nii, ofolder + "/noise_map.nii")
    if model == 'combine':
        S0init_map_nii = nib.Nifti1Image(S0init_map.copy(), mask_nii.affine, mask_nii.header); nib.save(S0init_map_nii, ofolder + "/S0init_map.nii")
        Dinit_map_nii = nib.Nifti1Image(Dinit_map.copy(), mask_nii.affine, mask_nii.header); nib.save(Dinit_map_nii, ofolder + "/Dinit_map.nii")
...
def fit_1shot_initD_v2(S, bvals, oplot_fname, n_vox_to_fit=1, true_params={}, verbose=1, bval_thr=200): #fit_1shot_initD_v2(S, bvals, oplot_fname, n_vox_to_fit=1, true_params={}, verbose=1, bval_thr=400):
    """
    ...
    try:
        ...
        ivim_params["Dstar"] = fit_res.params["Dstar"].value
        ivim_params["AIC"] = fit_res.aic
        ivim_params["AICc"] = fit_res.aic + (2*4**2 + 2*4) / (len(bvals) - 4)  # corrected AIC to take the small sample size into accout: AICc = AIC + (2k^2 + 2k) / n - k -1 where k is the number of parameters and n the sample size
        ivim_params["R2"] = get_r2(fit_res)
...
def fit_warpper(args_fit_func, approach):
    """
