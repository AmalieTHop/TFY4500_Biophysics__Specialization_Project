...
# import libraries
import numpy as np
import models.IVIMNET.IVIMNET.deep as deep #import IVIMNET.deep as deep
import models.IVIMNET.IVIMNET.fitting_algorithms as fit #import IVIMNET.fitting_algorithms as fit
import time
...
def sim(SNR, dst_dir, arg):
    ...
    # create directory
    snr_dir = os.path.join(dst_dir, f'SNR{SNR}')
    if not os.path.exists(snr_dir):
        os.makedirs(snr_dir)

    # this simulated the signal
    IVIM_signal_noisy, D, f, Dp = sim_signal(SNR, arg.sim.bvalues, sims=(arg.sim.num_samples_eval + arg.sim.sims), Dmin=arg.sim.range[0][0],
    ...
    IVIM_signal_noisy_test = IVIM_signal_noisy[:arg.sim.num_samples_eval]
    D_test = D[:arg.sim.num_samples_eval]
    Dp_test = Dp[:arg.sim.num_samples_eval]
    f_test = f[:arg.sim.num_samples_eval]

    np.save(os.path.join(snr_dir, f'IVIM_signal_noisy_SNR{SNR}_reps{arg.sim.repeats}'), IVIM_signal_noisy_test) ###
    np.save(os.path.join(snr_dir, f'Dt_gt_SNR{SNR}'), D_test)  ###
    np.save(os.path.join(snr_dir, f'Dp_gt_SNR{SNR}'), Dp_test) ###
    np.save(os.path.join(snr_dir, f'Fp_gt_SNR{SNR}'), f)  ###
    ...
    if not arg.train_pars.skip_net:
        ...
        for aa in range(arg.sim.repeats):
            ...
            print('\nRepeat: {repeat}\n'.format(repeat=aa))
            net = deep.learn_IVIM(IVIM_signal_noisy[arg.sim.num_samples_eval:, :], arg.sim.bvalues, arg)
        ...
        if arg.sim.repeats > 1:
            ...
            for aa in range(arg.sim.repeats):
                matNN[aa] = print_errors(np.squeeze(D_test), np.squeeze(f_test), np.squeeze(Dp_test), paramsNN[aa])
            np.save(os.path.join(snr_dir, f'mat_NN_SNR{SNR}_reps{arg.sim.repeats}_raw'), matNN) ###
            matNN = np.mean(matNN, axis=0)

            # calculate Stability Factor
            stability = np.sqrt(np.mean(np.square(np.std(paramsNN, axis=0)), axis=1))
            stability = stability[[0, 1, 2]] / [np.mean(D_test), np.mean(f_test), np.mean(Dp_test)]
            
            # set paramsNN for the plots
            paramsNN_0 = paramsNN[0]
        else:
            matNN = print_errors(np.squeeze(D_test), np.squeeze(f_test), np.squeeze(Dp_test), paramsNN)
            stability = np.zeros(3)
            paramsNN_0 = paramsNN

        # show figures if requested
        plots(arg, D_test, Dp_test, f_test, paramsNN_0)

        # save
        np.save(os.path.join(snr_dir, f'pred_params_NN_SNR{SNR}_reps{arg.sim.repeats}'), paramsNN) ###
        np.save(os.path.join(snr_dir, f'mat_NN_SNR{SNR}_reps{arg.sim.repeats}'), matNN) ###
        np.save(os.path.join(snr_dir, f'stability_SNR{SNR}_reps{arg.sim.repeats}'), stability) ###
    ...
    if arg.fit_conv.do_fit:
        start_time = time.time()
        # all fitting is done in the fit.fit_dats for the other fitting algorithms (conv and seg)
        paramsf = fit.fit_dats(arg.sim.bvalues, IVIM_signal_noisy[:arg.sim.num_samples_eval, :], arg.fit_conv)
        elapsed_time = time.time() - start_time
        print('\ntime elapsed for fit: {}\n'.format(elapsed_time))
        print('results for fit')

        # determine errors and Spearman Rank
        mat_conv = print_errors(np.squeeze(D_test), np.squeeze(f_test), np.squeeze(Dp_test), paramsf)

        # show figures if requested
        # plots(arg, D, Dp, f, paramsf, savename=f'conv')

        # save
        np.save(os.path.join(snr_dir, f'fitted_params_conv_SNR{SNR}'), paramsf) ###
        np.save(os.path.join(snr_dir, f'mat_conv_SNR{SNR}'), mat_conv) ###


    if arg.fit_seg.do_fit:
        start_time = time.time()
        # all fitting is done in the fit.fit_dats for the other fitting algorithms (conv and seg)
        paramsf = fit.fit_dats(arg.sim.bvalues, IVIM_signal_noisy[:arg.sim.num_samples_eval, :], arg.fit_seg)
        elapsed_time = time.time() - start_time
        print('\ntime elapsed for fit: {}\n'.format(elapsed_time))
        print('results for fit')

        # determine errors and Spearman Rank
        mat_seg = print_errors(np.squeeze(D_test), np.squeeze(f_test), np.squeeze(Dp_test), paramsf)

        # show figures if requested
        # plots(arg, D, Dp, f, paramsf, savename='seg')

        #save
        np.save(os.path.join(snr_dir, f'fitted_params_seg_SNR{SNR}'), paramsf) ###
        np.save(os.path.join(snr_dir, f'mat_seg_SNR{SNR}'), mat_seg)
