...
def checkarg(arg):
    ...
    if not hasattr(arg, 'sim'):
        ...
        arg.sim = sim()
    if not hasattr(arg, 'fit_conv'):
        warnings.warn('arg no conv. Using default initialisation')
        arg.fit_conv = conv_fit()
    if not hasattr(arg, 'fit_seg'):
        warnings.warn('arg no seg. Using default initialisation')
        arg.fit_seg = seg_fit()
    arg.net_pars=checkarg_net_pars(arg.net_pars)
    arg.train_pars = checkarg_train_pars(arg.train_pars)
    arg.sim = checkarg_sim(arg.sim)
    arg.fit_conv = fit.checkarg_lsq(arg.fit_conv)
    arg.fit_seg = fit.checkarg_lsq(arg.fit_seg)
    return arg
...
class net_pars:
    def __init__(self):
        ...
        self.cons_min = [0, 0, 0, 0.7]  # Dt, Fp, Ds, S0
...
class seg_fit:
    def __init__(self):
        self.method = 'seg' #seg, bayes or lsq
        self.do_fit = True # skip lsq fitting
        self.load_lsq = False # load the last results for lsq fit
        self.fitS0 = True # indicates whether to fit S0 (True) or fix it to 1 in the least squares fit.
        self.jobs = 4 # number of parallel jobs. If set to 1, no parallel computing is used
